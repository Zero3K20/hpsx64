...
// R5900_Execute.cpp
// TODO:
// 1. check for m-bit or run status on qmtc2.ni instruction
// 2. check for m-bit or run status on ctc2.ni instruction
...
// enable branch prediction timing for normal branches/jumps
#define ENABLE_R5900_BRANCH_PREDICTION_EXECUTE

// enable branch prediction timing for likely branches
#define ENABLE_R5900_BRANCH_PREDICTION_EXECUTE_LIKELY
...
#define USE_SCALAR_INTRINSICS_MUL_R5900
#define USE_SCALAR_INTRINSICS_MULA_R5900
#define USE_SCALAR_INTRINSICS_ADD_R5900
#define USE_SCALAR_INTRINSICS_ADDA_R5900
#define USE_SCALAR_INTRINSICS_SUB_R5900
#define USE_SCALAR_INTRINSICS_SUBA_R5900
#define USE_SCALAR_INTRINSICS_MADD_R5900
#define USE_SCALAR_INTRINSICS_MADDA_R5900
#define USE_SCALAR_INTRINSICS_MSUB_R5900
#define USE_SCALAR_INTRINSICS_MSUBA_R5900
...
#define ENABLE_GPR_REGISTER_TIMING_LOAD
//#define ENABLE_GPR_REGISTER_TIMING_STORE
...
template<const int ACCUM>
void Execute::EXECUTE_PS2_DOUBLE_MUL_SCALAR_AVX2(Instruction::Format i)
{
	static constexpr long long mantissa_double_mask = 0x000fffffe0000000ull;

	__m128i iop128_0, iop128_1, iop128_2, iop128_offset, iop128_sign, iop128_zero, iop128_overflow, iop128_underflow;
	...
	iop128_0 = _mm_insert_epi32(_mm_loadu_si32((const void*)&r->CPR1[i.Fs].s), *(long*)&r->CPR1[i.Ft].s, 1);
	//iop128_0 = _mm_load_si128((const __m128i*) & vs.uw0);
	//iop256_0 = _mm256_inserti128_si256(_mm256_castsi128_si256(iop128_0), _mm_load_si128((const __m128i*) & vt.uw0), 1);

	// convert to ps2 float/double

	// get offset
	//iop128_offset = _mm_srli_epi32(_mm_castps_si128(_mm_sub_ps(_mm_castsi128_ps(iop128_0), _mm_castsi128_ps(iop128_0))), 7);
	iop128_offset = _mm_slli_epi32(_mm_srli_epi32(_mm_slli_epi32(iop128_0, 1), 31), 23);


	// sub offset
	iop128_0 = _mm_sub_epi32(iop128_0, iop128_offset);




	// convert to double
	iop128_1 = _mm_add_epi64(
		_mm_slli_epi64(_mm_cvtepi32_epi64(iop128_offset), 29),
		_mm_castpd_si128(_mm_cvtps_pd(_mm_castsi128_ps(iop128_0)))
	);

	iop128_2 = _mm_shuffle_epi32(iop128_1, 0x0e);


	// do x*y adjustment
	iop128_2 = _mm_and_si128(iop128_2,
		_mm_add_epi64(iop128_2,
			_mm_cmpeq_epi64(_mm_and_si128(iop128_2,
				_mm_set1_epi64x(mantissa_double_mask)), _mm_set1_epi64x(mantissa_double_mask))));

	// do the double multiply
	iop128_0 = _mm_castpd_si128(_mm_mul_sd(
		_mm_castsi128_pd(iop128_1),
		_mm_castsi128_pd(iop128_2)
	));
	...
	// get offset
	iop128_offset = _mm_slli_epi64(_mm_srli_epi64(_mm_slli_epi64(iop128_0, 1), 63), 52);

	// convert to raw ps
	iop128_2 = _mm_castps_si128(_mm_cvtsd_ss(_mm_setzero_ps(), _mm_castsi128_pd(_mm_sub_epi64(iop128_0, iop128_offset))));

	// convert back to clamped pd
	iop128_1 = _mm_add_epi64(_mm_castpd_si128(_mm_cvtss_sd(_mm_setzero_pd(), _mm_castsi128_ps(iop128_2))), iop128_offset);

	// overflow is if clamped and unclamped masked pd don't match
	iop128_overflow = _mm_castpd_si128(
		_mm_cmp_sd(_mm_castsi128_pd(_mm_and_si128(iop128_0, iop128_1)), _mm_castsi128_pd(iop128_1), _CMP_NEQ_OQ)
	);

	// underflow
	iop128_underflow = _mm_castpd_si128(
		_mm_cmp_sd(_mm_cmp_sd(_mm_castsi128_pd(iop128_0), _mm_setzero_pd(), _CMP_EQ_OQ), _mm_castsi128_pd(iop128_1), _CMP_EQ_OQ)
	);

	// convert offset and offset ps
	iop128_offset = _mm_srli_epi64(iop128_offset, 29);

	// offset ps2 float
	iop128_0 = _mm_add_epi32(iop128_2, iop128_offset);
	...
	r->CPC1[31] &= ~0xc000;
	r->CPC1[31] |= (_mm_cvtsi128_si32(iop128_overflow) & 0x8010) | (_mm_cvtsi128_si32(iop128_underflow) & 0x4008);
	...
	if (ACCUM)
	{
		r->dACC.l = _mm_cvtsi128_si32(iop128_0);
	}
	else
	{
		r->CPR1[i.Fd].s = _mm_cvtsi128_si32(iop128_0);
	}
}


template<const int ACCUM, const int bSub>
void Execute::EXECUTE_PS2_DOUBLE_ADD_SCALAR_AVX2(Instruction::Format i)
{
	static constexpr long long mantissa_double_mask = 0x000fffffe0000000ull;
	...
	iop128_offset = _mm_xor_si128(iop128_1, iop128_2);

	if (bSub)
	{
		//e->pand2regreg(RBX, RAX, RCX);
		//iop128_offset = _mm_and_si128(iop128_1, iop128_2);
		iop128_offset = _mm_xor_si128(iop128_offset, _mm_set1_epi32(-1));

	}

	//e->psrlq2regimm(RBX, RBX, 63);
	//e->psllq2regimm(RBX, RBX, 27);
	iop128_offset = _mm_slli_epi64(_mm_srli_epi64(iop128_offset, 63), 27);
	...
	if (bSub)
	{
		...
		iop128_0 = _mm_castpd_si128(_mm_sub_sd(
			_mm_castsi128_pd(iop128_1),
			_mm_castsi128_pd(iop128_2)
		));
	}
	else
	{
		...
		//e->vaddpd2(RAX, RAX, RCX);
		// do the double add
		iop128_0 = _mm_castpd_si128(_mm_add_sd(
			_mm_castsi128_pd(iop128_1),
			_mm_castsi128_pd(iop128_2)
		));
	}
	...
	iop128_0 = _mm_add_epi64(iop128_0, iop128_offset);
...
template<const int ACCUM, const int bSub>
void Execute::EXECUTE_PS2_DOUBLE_MADD_SCALAR_AVX2(Instruction::Format i)
{
	static constexpr long long mantissa_double_mask = 0x000fffffe0000000ull;
	static constexpr long long max_double_float = 0x47ffffffe0000000ull;

	__m128i iop128_0, iop128_1, iop128_2, iop128_acc, iop128_offset, iop128_sign, iop128_zero, iop128_overflow, iop128_underflow;
	__m256i iop256_0, iop256_1, iop256_2, iop256_offset, iop256_offset0, iop256_offset1, iop256_overflow, iop256_underflow;
	//u32 u_ps2_stat_flags, u_ps2_mac_flags;
	...
	iop128_0 = _mm_insert_epi32(_mm_loadu_si32((const void*)&r->CPR1[i.Fs].s), *(long*)&r->CPR1[i.Ft].s, 1);
	iop128_0 = _mm_insert_epi32(iop128_0, *(long*)&r->dACC.l, 2);
	//iop128_0 = _mm_load_si128((const __m128i*) & vs.uw0);
	//iop256_0 = _mm256_inserti128_si256(_mm256_castsi128_si256(iop128_0), _mm_load_si128((const __m128i*) & vt.uw0), 1);
	...
	iop256_1 = _mm256_add_epi64(
		...
		_mm256_castpd_si256(_mm256_cvtps_pd(_mm_castsi128_ps(iop128_0)))
	);


	iop128_acc = _mm256_extracti128_si256(iop256_1, 1);

	iop128_1 = _mm256_castsi256_si128(iop256_1);
	...
	//u_ps2_stat_sticky_flags = _mm256_movemask_pd(_mm256_castsi256_pd(iop256_underflow)) ? 0x100 : 0;
	r->CPC1[31] |= _mm_cvtsi128_si32(iop128_underflow) & 0x8;


	// negate result if msub
	if (bSub)
	{
		iop128_1 = _mm_castpd_si128(_mm_sub_sd(_mm_setzero_pd(), _mm_castsi128_pd(iop128_1)));
	}
	...
	iop128_2 = _mm_blendv_epi8(iop128_acc, iop128_1, iop128_overflow);

	// check for acc overflow
	iop128_1 = _mm_blendv_epi8(
		iop128_1, iop128_2,
		_mm_cmpeq_epi64(_mm_and_si128(iop128_2, _mm_set1_epi64x(max_double_float)), _mm_set1_epi64x(max_double_float))
	);
...
void Execute::J ( Instruction::Format i )
{
	...
	//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPJ>;
	r->DelaySlot0.Data = (0xf0000000 & r->PC) | (i.JumpAddress << 2);
	r->Status.DelaySlot_Valid |= 0x2;

#ifdef ENABLE_R5900_BRANCH_PREDICTION_EXECUTE
	r->DelaySlot0_BranchTakenCycles = (r->DelaySlot0.Data & 0x4) ? r->c_ullLatency_BranchMisPredict : 0;
#else
	r->DelaySlot0_BranchTakenCycles = 0;
#endif
}
...
void Execute::JR ( Instruction::Format i )
{
	...
	// next instruction is in the branch delay slot
	//r->DelaySlot0.Instruction = i;
	//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPJR>;
	r->DelaySlot0.Data = r->GPR[i.Rs].u & ~3;
	r->Status.DelaySlot_Valid |= 0x2;

#ifdef ENABLE_R5900_BRANCH_PREDICTION_EXECUTE
	r->DelaySlot0_BranchTakenCycles = r->c_ullLatency_BranchMisPredict;
#else
	r->DelaySlot0_BranchTakenCycles = 0;
#endif
}
...
void Execute::JAL ( Instruction::Format i )
{
	...
	//r->DelaySlot0.Instruction = i;
	//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPJAL>;
	r->DelaySlot0.Data = (0xf0000000 & r->PC) | (i.JumpAddress << 2);
	r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::JALR ( Instruction::Format i )
{
	...
	// next instruction is in the branch delay slot
	//r->DelaySlot0.Instruction = i;
	//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPJALR>;
	r->DelaySlot0.Data = r->GPR[i.Rs].u & ~3;
	r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BEQ ( Instruction::Format i )
{
	...
	if ( r->GPR [ i.Rs ].u == r->GPR [ i.Rt ].u )
	{
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBEQ>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BNE ( Instruction::Format i )
{
	...
	if ( r->GPR [ i.Rs ].u != r->GPR [ i.Rt ].u )
	{
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBNE>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BLEZ ( Instruction::Format i )
{
	...
	if ( r->GPR [ i.Rs ].s <= 0 )
	{
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBLEZ>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BGTZ ( Instruction::Format i )
{
	...
	if ( r->GPR [ i.Rs ].s > 0 )
	{
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBGTZ>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BLTZ ( Instruction::Format i )
{
	...
	if ( r->GPR [ i.Rs ].s < 0 )
	{
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBLTZ>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BGEZ ( Instruction::Format i )
{
	...
	if ( r->GPR [ i.Rs ].s >= 0 )
	{
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBGEZ>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BLTZAL ( Instruction::Format i )
{
	...
	if ( r->GPR [ i.Rs ].s < 0 )
	{
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBLTZAL>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BGEZAL ( Instruction::Format i )
{
	...
	if ( r->GPR [ i.Rs ].s >= 0 )
	{
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBGEZAL>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::CFC2_I ( Instruction::Format i )
{
	...
	if ( VU0::_VU0->VifRegs.STAT.VEW )
	{
		...
		// also, tell vu0 to force completion
		r->Status.Force_Vu0_Finish = 1;
#endif
	...
	else
	{
		...
		//VU0::_VU0->CycleCount = r->CycleCount;

		// update q reg and div pipeline flags
		//VU0::_VU0->UpdateQ();
...
u64* Execute::handle_cached_load ( u32 LoadAddress, u32 BaseRegister )
{
	...
	debug << "+MISS";
			...
			if (
				// check for cache-miss already in progress
				( r->CycleCount < r->LoadFromBus_BusyUntilCycle )
				//||
				// check for a non-gpr register
				//((i.Opcode == OPLQC2) || (i.Opcode == OPLWC1))
				)
			{
				// blocking load //
				
#ifdef ENABLE_BUS_SIMULATION_CACHE_LOAD
				// wait until the pending cache data has loaded
				...
				// update the time to refill the cache
				//r->CycleCount += r->c_ullCacheRefill_CycleTime;

				// the bus is occupied while the cache data is transferring
			...
			else
			{
				...
				// cache-miss is now in progress
				r->LoadFromBus_BusyUntilCycle = r->CycleCount + r->Bus->c_iRAM_Read_Latency;

				// update cycles if bus is currently otherwise occupied
				...
				// bus will be occupied until data is loaded
				...
				r->DCache.busyUntilCycles64[ulIndex] = r->LoadFromBus_BusyUntilCycle;
	...
	debug << "+HIT";
			...
			if (r->CycleCount < r->DCache.busyUntilCycles64[ulIndex])
			{
				// wait until the cache-line is filled from memory
				r->CycleCount = r->DCache.busyUntilCycles64[ulIndex];
			}
#endif
...
u64* Execute::handle_cached_store ( u32 StoreAddress )
{
	...
	debug << "+MISS";
			...
			// check if cpu is already accessing bus -> blocking store
			if (
				// check for cache-miss already in progress
				(r->CycleCount < r->LoadFromBus_BusyUntilCycle)
				)
			{
				...
				// update the cycle count to wait for cache data to be loaded
				r->CycleCount = r->LoadFromBus_BusyUntilCycle + r->Bus->c_iRAM_Read_Latency;

				// wait for bus to be free
				if ( r->CycleCount < r->Bus->BusyUntil_Cycle ) r->CycleCount = r->Bus->BusyUntil_Cycle;

				// update with time to refill the cache
				//r->CycleCount += r->c_ullCacheRefill_CycleTime;

				// bus is busy until this time
				r->Bus->BusyUntil_Cycle = r->CycleCount;
#else
				r->CycleCount = r->LoadFromBus_BusyUntilCycle + r->Bus->c_iRAM_Read_Latency + r->c_ullCacheRefill_CycleTime;
#endif
			...
			else
			{
				...
				// set time that d-cache is loading from memory
				r->LoadFromBus_BusyUntilCycle = r->CycleCount + r->Bus->c_iRAM_Read_Latency;

				// update with time that bus is occupied
				if ( r->LoadFromBus_BusyUntilCycle < r->Bus->BusyUntil_Cycle ) r->LoadFromBus_BusyUntilCycle = r->Bus->BusyUntil_Cycle;


				// update with time to load into cache?
				//r->LoadFromBus_BusyUntilCycle += r->c_ullCacheRefill_CycleTime;

				// set the time that bus is occupied
				r->Bus->BusyUntil_Cycle = r->LoadFromBus_BusyUntilCycle;
#else
				r->LoadFromBus_BusyUntilCycle = r->CycleCount + r->Bus->c_iRAM_Read_Latency + r->c_ullCacheRefill_CycleTime;
#endif
...
void Execute::BEQL ( Instruction::Format i )
{
	...
	if ( r->GPR [ i.Rs ].u == r->GPR [ i.Rt ].u )
	{
		...
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBEQL>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
		
#ifdef ENABLE_R5900_BRANCH_PREDICTION_EXECUTE_LIKELY
		r->DelaySlot0_BranchTakenCycles = (r->DelaySlot0.Data & 0x4) ? r->c_ullLatency_BranchMisPredict : 0;
#else
		r->DelaySlot0_BranchTakenCycles = 0;
#endif
...
void Execute::BNEL ( Instruction::Format i )
{
	...
	if ( r->GPR [ i.Rs ].u != r->GPR [ i.Rt ].u )
	{
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBNEL>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BGEZL ( Instruction::Format i )
{
	...
	if ( r->GPR [ i.Rs ].s >= 0 )
	{
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBGEZL>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BLEZL ( Instruction::Format i )
{
	...
	if ( r->GPR [ i.Rs ].s <= 0 )
	{
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBLEZL>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BGTZL ( Instruction::Format i )
{
	...
	if ( r->GPR [ i.Rs ].s > 0 )
	{
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBGTZL>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BLTZL ( Instruction::Format i )
{
	...
	if ( r->GPR [ i.Rs ].s < 0 )
	{
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBLTZL>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BLTZALL ( Instruction::Format i )
{
	...
	if ( r->GPR [ i.Rs ].s < 0 )
	{
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBLTZALL>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BGEZALL ( Instruction::Format i )
{
	...
	if ( r->GPR [ i.Rs ].s >= 0 )
	{
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBGEZALL>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BC0T ( Instruction::Format i )
{
	...
	if ( r->CPCOND0 )
	{
		...
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBC0T>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BC0TL ( Instruction::Format i )
{
	...
	if ( r->CPCOND0 )
	{
		...
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBC0TL>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BC0F ( Instruction::Format i )
{
	...
	if ( !r->CPCOND0 )
	{
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBC0F>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BC0FL ( Instruction::Format i )
{
	...
	if ( !r->CPCOND0 )
	{
		...
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBC0FL>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BC1T ( Instruction::Format i )
{
	...
	if ( r->CPC1 [ 31 ] & 0x00800000 )
	{
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBC1T>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BC1TL ( Instruction::Format i )
{
	...
	if ( r->CPC1 [ 31 ] & 0x00800000 )
	{
		...
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBC1TL>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BC1F ( Instruction::Format i )
{
	...
	if ( ! ( r->CPC1 [ 31 ] & 0x00800000 ) )
	{
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBC1F>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BC1FL ( Instruction::Format i )
{
	...
	if ( ! ( r->CPC1 [ 31 ] & 0x00800000 ) )
	{
		...
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBC1FL>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BC2T ( Instruction::Format i )
{
	...
	if ( VU1::_VU1->Running )
	{
		...
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBC2T>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BC2TL ( Instruction::Format i )
{
	...
	if ( VU1::_VU1->Running )
	{
		...
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBC2TL>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BC2F ( Instruction::Format i )
{
	...
	if ( !VU1::_VU1->Running )
	{
		...
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBC2F>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
void Execute::BC2FL ( Instruction::Format i )
{
	...
	if ( !VU1::_VU1->Running )
	{
		...
		// next instruction is in the branch delay slot
		//r->DelaySlot0.Instruction = i;
		//r->DelaySlot0.cb = r->ProcessBranchDelaySlot_t<OPBC2FL>;
		r->DelaySlot0.Data = 4 + r->PC + (i.sImmediate << 2);
		r->Status.DelaySlot_Valid |= 0x2;
...
#ifdef USE_SCALAR_INTRINSICS_ADD_R5900

	EXECUTE_PS2_DOUBLE_ADD_SCALAR_AVX2<false,false>(i);
...
#ifdef USE_SCALAR_INTRINSICS_ADDA_R5900

	EXECUTE_PS2_DOUBLE_ADD_SCALAR_AVX2<true, false>(i);
...
#ifdef USE_SCALAR_INTRINSICS_SUB_R5900

	EXECUTE_PS2_DOUBLE_ADD_SCALAR_AVX2<false, true>(i);
...
#ifdef USE_SCALAR_INTRINSICS_MUL_R5900

	EXECUTE_PS2_DOUBLE_MUL_SCALAR_AVX2<false>(i);
...
#ifdef USE_SCALAR_INTRINSICS_MULA_R5900

	EXECUTE_PS2_DOUBLE_MUL_SCALAR_AVX2<true>(i);
...
#ifdef USE_SCALAR_INTRINSICS_SUBA_R5900

	EXECUTE_PS2_DOUBLE_ADD_SCALAR_AVX2<true, true>(i);
...
#ifdef USE_SCALAR_INTRINSICS_MADD_R5900

	EXECUTE_PS2_DOUBLE_MADD_SCALAR_AVX2<false, false>(i);
...
#ifdef USE_SCALAR_INTRINSICS_MSUB_R5900

	EXECUTE_PS2_DOUBLE_MADD_SCALAR_AVX2<false, true>(i);
...
#ifdef USE_SCALAR_INTRINSICS_MSUBA_R5900

	EXECUTE_PS2_DOUBLE_MADD_SCALAR_AVX2<true, true>(i);
...
#ifdef USE_SCALAR_INTRINSICS_MADDA_R5900

	EXECUTE_PS2_DOUBLE_MADD_SCALAR_AVX2<true, false>(i);
...
void Execute::VRNEXT ( Instruction::Format i )
{
	...
	{
	i.Value = 0x80000000 | ( i.Value & 0x01ffffff );
	Vu::Instruction::Execute::VRNEXT ( VU0::_VU0, (Vu::Instruction::Format&) i );
...
void Execute::VMFIR ( Instruction::Format i )
{
	...
	{
	i.Value = 0x80000000 | ( i.Value & 0x01ffffff );
	Vu::Instruction::Execute::VMFIR ( VU0::_VU0, (Vu::Instruction::Format&) i );
...
void Execute::VRGET ( Instruction::Format i )
{
	...
	{
	i.Value = 0x80000000 | ( i.Value & 0x01ffffff );
	Vu::Instruction::Execute::VRGET ( VU0::_VU0, (Vu::Instruction::Format&) i );
