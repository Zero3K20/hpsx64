...
//#pragma once
#ifndef __R5900_DCACHE_H__
#define __R5900_DCACHE_H__
...
namespace R5900
{
	...
	public:
		...
		// to determine which way to use
		// (((LRF0 ^ LRF1) | LOCK0) & ~LOCK1) | ?? VALID0 VALID1

		// size of d-cache in bytes
		// DCache on R5900 is 8KB
		static constexpr u32 DCACHE_SIZE_BYTES = 8192;
		
		// 64 bytes per way on R5900 cache
		static constexpr u32 DCACHE_LINE_SIZE_BYTES = 64;
		
		// the number of cache lines
		static constexpr u32 DCACHE_LINE_COUNT = DCACHE_SIZE_BYTES / DCACHE_LINE_SIZE_BYTES;

		// marks if a cache line is valid or not
		alignas(32) u8 Valid[DCACHE_LINE_COUNT];
		
		// flips when data is loaded
		alignas(32) u8 LRF[DCACHE_LINE_COUNT];

		// marks if a cache line is dirty/modified or not
		alignas(32) u8 Dirty[DCACHE_LINE_COUNT];

		// marks if a cache line is locked or not
		alignas(32) u8 Lock[DCACHE_LINE_COUNT];
		...
		alignas(32) u32 PFN[DCACHE_LINE_COUNT];
		
		// the data in ICache
		// has 1024 entries
		alignas(32) u32 Data[DCACHE_SIZE_BYTES / sizeof(u32)];

		// the cycle# at which the data will be available in the d-cache
		alignas(32) uint64_t busyUntilCycles64[DCACHE_LINE_COUNT];

		// the cycle# the last cache miss occurred at
		// if there is miss under miss, then I guess the d-cache isn't ready until this time?
		//uint64_t lastCacheMiss_BusyUntilCycle;
		...
		void Reset()
		{
			...
			memset(PFN, -1, DCACHE_LINE_COUNT * sizeof(u32));
		}
...
#endif	// end #ifndef __R5900_DCACHE_H__
