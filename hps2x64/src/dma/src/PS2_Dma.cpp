...
// subtract bus time from cpu time
//#define ENABLE_DMA_CPU_CYCLE_TIME
//#define ENABLE_DMA_CPU_CYCLE_TIME_SPROUT
//#define ENABLE_DMA_CPU_CYCLE_TIME_SPRIN


// limit transfer size to device buffer size for normal transfer
#define LIMIT_NORMAL_TRANSFER_SIZE


// split transfers outside of memory bounds when it hits the memory bounds
// note: currently only handles main memory bounds
#define LIMIT_OUT_OF_BOUNDS_TRANSFERS_CHAIN
#define LIMIT_OUT_OF_BOUNDS_TRANSFERS_NORMAL
...
#define ALLOW_VERBOSE_DEBUG_PS2_DMA_READ
#define ALLOW_VERBOSE_DEBUG_PS2_DMA_WRITE
...
#define INLINE_DEBUG_GETCHANNELPRIORITY
...
bool Dma::bEnableVerboseDebugRead;
bool Dma::bEnableVerboseDebugWrite;
...
const u32 Dma::c_iStallSource_LUT[4] = { (u32)-1, 5, 8, 3 };
const u32 Dma::c_iStallDest_LUT[4] = { (u32)-1, 1, 2, 6 };
const u32 Dma::c_iMfifoDrain_LUT[4] = { (u32)-1, (u32)-1, 1, 2 };
...
bool Dma::isEnabled(int iChannel)
{
	...
	// dma channel appears to be enabled
	return true;
}
...
u32 Dma::Get_ChannelPriority ( int iChannel )
{
	...
	if ( ! pDMARegs->CTRL.DMAE )
	{
#ifdef INLINE_DEBUG_GETCHANNELPRIORITY
		debug << " !DMAE ";
#endif
	...
	*/
	...
	if ( pRegData [ iChannel ]->CHCR.DIR )
	{
		...
		{
#ifdef INLINE_DEBUG_GETCHANNELPRIORITY
			debug << " CALLING:" << hex << (u64)(cbReady[iChannel]);
#endif
	...
	else
	{
		...
		{
#ifdef INLINE_DEBUG_GETCHANNELPRIORITY
			debug << " CALLING:" << hex << (u64)(cbReady_ToMemory[iChannel]);
#endif
...
u64 Dma::Read ( u32 Address, u64 Mask )
{
...
#ifdef ALLOW_VERBOSE_DEBUG_PS2_DMA_READ
	if (bEnableVerboseDebugRead)
	{
		cout << "\nPS2 DMA READ ADDR:0x" << hex << Address;

		if ((Address & 0xffff) < 0xffff && c_ucDMAChannel_LUT[(Address >> 10) & 0x1f] != 0xff)
		{
			cout << " " << REG_NAMES[c_ucDMAChannel_LUT[(Address >> 10) & 0x1f]][(Address >> 4) & 0xf];
		}
		else
		{
			cout << " ERROR";
...
#endif	// end #ifdef ALLOW_VERBOSE_DEBUG_PS2_DMA_READ
	...
	if (bEnableVerboseDebugRead)
	{
		cout << " DATA:0x" << hex << Output64 << " MASK:0x" << Mask << " PC:0x" << *_DebugPC << " CYCL#" << dec << *_DebugCycleCount;
	}
#endif
...
void Dma::Write ( u32 Address, u64 Data, u64 Mask )
{
	...
	static const u32 c_ulChcrAND[10] = { (u32)-1, (u32)-1, (u32)-1, (u32)~1, (u32)-1, (u32)~1, (u32)-1, (u32)-1, (u32)~1, (u32)-1 };
...
#ifdef ALLOW_VERBOSE_DEBUG_PS2_DMA_WRITE
	if (bEnableVerboseDebugWrite)
	{
		cout << "\nPS2 DMA WRITE ADDR:0x" << hex << Address;
...
#endif	// end #ifdef ALLOW_VERBOSE_DEBUG_PS2_DMA_WRITE
	...
	if ( Address < 0x8000 )
	{
		...
		if ( ( Address & 0xf0 ) == 0x10 )
		{
			if ( Address < ( ( DMA_CTRL & 0xffff ) - 0x8000 ) )
			{
				...
				// check if dma is running already
				if (pRegData[DmaChannelNum]->CHCR.STR)
				{
					// check if dma has been halted/stopped
					if (!(pDMARegs->ENABLER & 0x10000))
					{
						cout << "\nhps2x64: PS2: DMA: Writing to MADR while DMA is NOT halted by ENABLER/W.";
						cout << "\nChan#" << dec << DmaChannelNum << " DATA:" << hex << Data << " PREV:" << PreviousValue << " NEW:" << pDMARegs->Regs[Offset];
						//cout << " CycleStealMode: " << hex << pDMARegs->CTRL.CycleStealMode << " RCYC: " << pDMARegs->CTRL.RCYC;
						//cout << " QWTotal: " << dec << _DMA->QWC_BlockTotal[DmaChannelNum] << " QWLeft: " << _DMA->QWC_Transferred[DmaChannelNum];
						//cout << " QWC: " << pRegData[DmaChannelNum]->QWC.QWC;
					}
				}
			...
			if ( Address < ( ( DMA_CTRL & 0xffff ) - 0x8000 ) )
			{
				...
				if (PreviousValue & 0x100)
				{
					...
					if (!(pDMARegs->ENABLER & 0x10000))
					{
						...
						// only modify chcr
						//pDMARegs->Regs[Offset] = (pDMARegs->Regs[Offset] & 0x040) | (PreviousValue & ~0x040);

						//return;

						// will keep a watch on this
						// looks like some faster non-chain dma#1 or dma#9 etc transfers are not completing soon enough
						cout << "\nhps2x64: PS2: DMA: Writing to CHCR while DMA is NOT halted by ENABLER/W.";
						cout << "\nChan#" << dec << DmaChannelNum << " DATA:" << hex << Data << " PREV:" << PreviousValue << " NEW:" << pDMARegs->Regs[Offset];
						cout << " CycleStealMode: " << hex << pDMARegs->CTRL.CycleStealMode << " RCYC: " << pDMARegs->CTRL.RCYC;
						cout << " QWTotal: " << dec << _DMA->QWC_BlockTotal[DmaChannelNum] << " QWLeft: " << _DMA->QWC_Transferred[DmaChannelNum];
						cout << " QWC: " << pRegData[DmaChannelNum]->QWC.QWC;
...
#ifdef ENABLE_DMA_CPU_CYCLE_TIME

					// subtract time from cpu
					R5900::Cpu::_CPU->CycleCount += TransferCount;
...
void Dma::NormalTransfer_ToMemory ( int iChannel )
{
	...
	debug << "; TransferingData";
		...
		QWC_TransferCount = QWC_BlockTotal[iChannel] - QWC_Transferred[iChannel];

#ifdef LIMIT_NORMAL_TRANSFER_SIZE
...
#ifdef LIMIT_OUT_OF_BOUNDS_TRANSFERS_NORMAL

		QWC_TransferCount = (((pRegData[iChannel]->MADR.Address & DataBus::MainMemory_Mask) + (QWC_TransferCount << 4)) > DataBus::MainMemory_Mask) ? ((DataBus::MainMemory_Size - (pRegData[iChannel]->MADR.Address & DataBus::MainMemory_Mask)) >> 4) : QWC_TransferCount;
	...
	debug << " STADR=" << DMARegs.STADR;
		...
		*/
		...
		// subtract time from cpu
		R5900::Cpu::_CPU->CycleCount += QWC_TransferCount;
...
#ifdef LIMIT_OUT_OF_BOUNDS_TRANSFERS_CHAIN
...
#ifdef ENABLE_DMA_CPU_CYCLE_TIME_SPROUT

	// subtract time from cpu
	R5900::Cpu::_CPU->CycleCount += QuadwordCount;
...
#ifdef ENABLE_DMA_CPU_CYCLE_TIME_SPRIN
...
u64* Dma::GetMemoryPtr ( u32 Address )
{
	...
	if (Address < 0x10000000)
	{
#ifdef VERBOSE_PS2_DMA_ZEROS_RW
...
void Dma::DebugWindow_Enable ()
{
	...
	static constexpr const char* DebugWindow_Caption = "PS2 DMA Debug Window";
