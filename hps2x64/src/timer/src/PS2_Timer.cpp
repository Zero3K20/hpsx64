...
// mask final timer value before returning it
//#define ENABLE_MASK_FINAL_TIMER_VALUES

#define ALLOW_VERBOSE_DEBUG_PS2_TIMER_READ
#define ALLOW_VERBOSE_DEBUG_PS2_TIMER_WRITE
...
uint64_t *Timer::_llCycleCount;
uint64_t *Timer::_llScanlineStart, *Timer::_llNextScanlineStart, *Timer::_llHBlankStart;
uint32_t *Timer::_lScanline, *Timer::_lNextScanline, *Timer::_lVBlank_Y, *Timer::_lRasterYMax;
...
u64 Timers::Read ( u32 Address, u64 Mask )
{
...
#ifdef ALLOW_VERBOSE_DEBUG_PS2_TIMER_READ
	if (bEnableVerboseDebugRead)
	{
		cout << "\nPS2 DMA READ ADDR:0x" << hex << Address;

		if ((Address & 0xffff) < 0x18ff && ((Address >> 11) & 0x1f) <= 3 && ((Address >> 4) & 0x7f) < 4)
		{
			cout << " " << REG_NAMES[(Address >> 11) & 0x3][(Address >> 4) & 0x3];
		}
		else
		{
			cout << " ERROR";
...
#endif	// end #ifdef ALLOW_VERBOSE_DEBUG_PS2_TIMER_READ
	...
	switch ( ( Address >> 4 ) & 0xf )
	{
		case 0:	// READ: COUNT
			...
			Output = _TIMERS->TheTimers[TimerNumber].COUNT.Value & 0xffff;
#else
			Output = _TIMERS->TheTimers [ TimerNumber ].COUNT.Value;
#endif
			break;
		...
		case 2:	// READ: COMP
			...
			Output = _TIMERS->TheTimers [ TimerNumber ].COMP.Value;
			break;
		...
		case 3:	// READ: HOLD
			...
			if ( TimerNumber <= 1 )
			{
				Output = _TIMERS->TheTimers [ TimerNumber ].HOLD.Value;
			}
...
#ifdef ALLOW_VERBOSE_DEBUG_PS2_TIMER_WRITE
	if (bEnableVerboseDebugWrite)
	{
		cout << "\nPS2 TIMER WRITE ADDR:0x" << hex << Address;
...
#endif	// end #ifdef ALLOW_VERBOSE_DEBUG_PS2_TIMER_WRITE
...
u64 Timer::CountTicks ( u64 lBeginCycle, u64 lEndCycle )
{
	...
	switch ( MODE.ClkSelect )
	{
		...
		case 3:
			...
			// hblank only happens outside of vblank ??
...
void Timers::DebugWindow_Enable ()
{
	...
	static constexpr const char* DebugWindow_Caption = "PS2 Timer Debug Window";
