...
//#pragma once
#ifndef __PS2_SIF_H__
#define __PS2_SIF_H__
...
namespace Playstation2
{
	...
	public:
		...
		// 0x1000f200
		static constexpr char* REG_NAMES_EE[8] = { "EE_SIF_CMD", "EE_SIF_HCMD", "EE_SIF_STAT", "EE_SIF_SMFLAG", "EE_SIF_SMADDR", "EE_SIF_RCMD", "EE_SIF_RPC", "EE_SIF_UNK"};

		// 0x1d000000
		static constexpr char* REG_NAMES_IOP[8] = { "IOP_SIF_CMD", "IOP_SIF_HCMD", "IOP_SIF_STAT", "IOP_SIF_SMFLAG", "IOP_SIF_SMADDR", "IOP_SIF_RCMD", "IOP_SIF_RPC", "IOP_SIF_UNK"};
		...
		// SIF EE Data Out/IOP Data In - Command Register - (transfers data from EE->IOP)
		static const long EESIF_F200 = 0x1000f200;
		
		// SIF EE Data In/IOP Data Out - Host Command Register - (transfers data from IOP->EE)
		static const long EESIF_F210 = 0x1000f210;
		
		// SIF EE Flags Out - Status Register - bits get set from EE side on write and cleared from IOP side on write
		static const long EESIF_F220 = 0x1000f220;
		
		// SIF IOP Flags Out - Shared Memory Flag - bits get set from IOP side on write and cleared from EE side on write
		static const long EESIF_F230 = 0x1000f230;

		// SIF CTRL - Control Register - Shared Memory Address
		static const long EESIF_CTRL = 0x1000f240;
		
		// SIF Receive Command Register (IOP -> EE)
		static const long EE_SIF_F250 = 0x1000f250;

		// SIF F260 - F260 Register - RPC Trigger Register
		static const long EESIF_F260 = 0x1000f260;
		
		
		// Command Register - Command sent to IOP - (transfers data from EE->IOP)
		// bits 0-7: Command ID
		// bits 8-31: Optional parameters or flags
		static constexpr long EE_SIF_CMD = 0x1000f200;

		// Host Command Register - used in debugging or host communication - (transfers data from IOP->EE)
		static constexpr long EE_SIF_HCMD = 0x1000f210;

		// Status Register - bits get set from EE side on write and cleared from IOP side on write
		// bit 0: EE to IOP command ready
		// bit 1: IOP to EE command received
		// bit 2: DMA transfer in progress
		// bit 3: RPC active
		static constexpr long EE_SIF_STAT = 0x1000f220;

		// Shared Memory Flag - bits get set from IOP side on write and cleared from EE side on write
		// bit 0: EE has written to shared memory
		// bit 1: IOP has acknowledged
		// bit 2: lock flag
		static constexpr long EE_SIF_SMFLAG = 0x1000f230;

		// Shared Memory Address - Physical address of shared memory used for RPC or data exchange
		static constexpr long EE_SIF_SMADDR = 0x1000f240;

		// SIF Receive Command Register (IOP -> EE) - Receive command from IOP
		// bits 0-7: Command ID
		// bits 8-31: parameters or status
		static constexpr long EE_SIF_RCMD = 0x1000f250;

		// RPC Trigger Register
		// bit 0: start rpc
		// bit 1: rpc complete
		// bit 2: error flag
		static constexpr long EE_SIF_RPC = 0x1000f260;
...
#endif	// end #ifndef __PS2_SIF_H__
