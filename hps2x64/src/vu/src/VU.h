...
// use cycle number as flag index
//#define USE_CYCLE_NUMBER_FLAG_INDEX
...
namespace Playstation2
{
	...
	public:
		...
		// power of 2 of the vu 0/1 code block size
		static constexpr const int VU0_CODE_BLOCK_SIZE_SHIFT = 19;
		static constexpr const int VU1_CODE_BLOCK_SIZE_SHIFT = 21;
		...
		*/
		...
		typedef void (*ps2_vu_func_template) (VU* v, u32* &Data, uint32_t& lWordsLeft, uint32_t& ulPackedSize);
		static ps2_vu_func_template ps2_arr_execute_unpack_lut[1 << 8];

		typedef void (*ps2_vu_func_template2) (VU* v, u8*& Data, uint32_t& lWordsLeft, uint32_t& ulPackedSize);
		static ps2_vu_func_template2 ps2_arr_execute_unpack_avx2_lut[1 << 8];

		// luts for the unpack masks
		alignas(16) static u32 ps2_arr_mask_row_avx2_lut[256 << 2];
		alignas(16) static u32 ps2_arr_mask_col_avx2_lut[256 << 2];
		alignas(16) static u32 ps2_arr_mask_write_avx2_lut[256 << 2];
		alignas(16) static u32 ps2_arr_mask_rowwrite_avx2_lut[256 << 2];
		...
		// mxcsr register value
		long mxcsr_flag_value;
		...
		// the next hi/lo instruction during encoding
		Vu::Instruction::Format NextInstLO;
		Vu::Instruction::Format NextInstHI;

		// the next hi/lo instruction during encoding/recompiling
		Vu::Instruction::Format64 NextInst;

		// the previous hi/lo instruction during encoding/recompiling
		Vu::Instruction::Format64 PrevInst;

		Vu::Instruction::Format64 PrevInst2;
		Vu::Instruction::Format64 PrevInst3;
		...
		// the address where the last continuous execution (sequential execution, w/o jumps) started
		// note: this is used to determine what instructions were executed in the continuous run so far
		u32 ulRunStartAddress;
		...
		*/
		...
		inline void SetQ ()
		{
			...
			// clear sticky bits in case they get set via stat set
			NextQ_Flag &= ~0xc00;
		...
		static void Perform_UnJump ( VU* v, Vu::Instruction::Format i );
		...
		inline void UpdateFlags ()
		{
			...
			int FlagIndex = CycleCount & c_lFlag_Delay_Mask;
#else
			// this can use the cycle counter later possibly
			iFlagSave_Index++;
			
			// set the flags
			int FlagIndex = iFlagSave_Index & c_lFlag_Delay_Mask;
#endif
			...
			// set cycle#
			FlagSave[FlagIndex].ullBusyUntil_Cycle = CycleCount;
		}
		...
		// is this flag to determine if waiting for the end of the microprogram (meaning vu only needs to stop before using path 1)
		u32 VifWaiting;
		...
		// need the bytes remaining to be read
		u32 ulReadByteCount;
		u32 ulLastReadByteCount;

		// remaining data in unpack
		u64 ullLastUnpackData;
		__m128i qwLastUnpackData;
		...
		static void Execute_UNPACK(VU* v, u32* &Data, uint32_t& lWordsLeft, uint32_t& ulPackedSize);

		template<const int VIFREG_MODE, const int VIFCODE_USN, const int VIFCODE_M, const int VIFCODE_VN, const int VIFCODE_VL>
		static void Execute_UNPACK_AVX2(VU* v, u8*& Data8, uint32_t& lWordsLeft, uint32_t& ulPackedSize);
		...
		// need a place to hold arguments for recompiler when doing an x2 or x4 instruction (avx2/avx512)
		alignas(64) Reg128 xargs0[4];
		alignas(64) Reg128 xargs1[4];
		alignas(64) Reg128 xresult[4];
		alignas(64) Reg128 xoverflow[4];
		alignas(8) u32 xcomp[2];
		...
		static constexpr char* REG_NAMES[][32] = {
			// VIF0 0x10003800
			{"VIF0_STAT", "VIF0_FBRST", "VIF0_ERR", "VIF0_MARK", "VIF0_CYCLE", "VIF0_MODE", "VIF0_NUM", "VIF0_MASK",
			"VIF0_CODE", "VIF0_ITOPS", "VIF0_RES", "VIF0_RES", "VIF0_RES", "VIF0_ITOP", "VIF0_RES", "VIF0_RES",
			"VIF0_R0", "VIF0_R1","VIF0_R2","VIF0_R3","VIF0_C0","VIF0_C1","VIF0_C2","VIF0_C3",
			"VIF0_RES","VIF0_RES","VIF0_RES","VIF0_RES","VIF0_RES","VIF0_RES","VIF0_RES","VIF0_RES"},

			// VIF1 0x10003c00
			{"VIF1_STAT", "VIF1_FBRST", "VIF1_ERR", "VIF1_MARK", "VIF1_CYCLE", "VIF1_MODE", "VIF1_NUM", "VIF1_MASK",
			"VIF1_CODE", "VIF1_ITOPS", "VIF1_BASE", "VIF1_OFST", "VIF1_TOPS", "VIF1_ITOP", "VIF1_TOP", "VIF1_RES",
			"VIF1_R0", "VIF1_R1","VIF1_R2","VIF1_R3","VIF1_C0","VIF1_C1","VIF1_C2","VIF1_C3",
			"VIF1_RES","VIF1_RES","VIF1_RES","VIF1_RES","VIF1_RES","VIF1_RES","VIF1_RES","VIF1_RES"}
		};

		static constexpr char* REG_NAMES2[] = { "VIF0_FIFO, VIF1_FIFO" };

		static bool bEnableVerboseDebugRead;
		static bool bEnableVerboseDebugWrite;
		...
		struct DelaySlot
		{
			union
			{
				struct
				{
					Vu::Instruction::Format Instruction;
					u32 Data;
					cbFunction cb;
				};
		...
		{
			UpperDest_Bitmap |= ( 1ull << Register );
		}
		...
		*/
		...
		// the latest cycle# that a component for float reg is busy until
		// can be used for a quick check via recompiler if all components are in use
		// can also be used for vu0 macro mode
		alignas(64) u64 FReg_BusyUntil[32];

		// cycle# each float register component is busy until
		// used to determine if float reg component is available or busy in pipeline
		alignas(64) u64 FRegXYZW_BusyUntil[32 * 4];

		// cycle# that integer register is available at - like if it is being loaded
		alignas(64) u64 IReg_BusyUntil[32];

		//u32 FReg_BusyUntil_Bitmap[32];

		// pipeline data for float source registers
		int iFSrcReg_Count;
		u8 uFSrcReg_Index8[8];
		u8 uFSrcReg_Mask8[8];

		// pipeline data for int source registers
		int iISrcReg_Count;
		u8 uISrcReg_Index8[8];

		// pipeline data for float dst registers
		int iFDstReg_Count;
		u8 uFDstReg_Index8[8];
		u8 uFDstReg_Mask8[8];

		// pipeline data for int dst registers
		int iIDstReg_Count;
		u8 uIDstReg_Index8[8];

		// mac+stat and cycle#
		// these are circular buffers of 4 elements each
		u32 MSReg_Index;
 		u64 MSReg_BusyUntil_Cycle[4];
		u32 Reg_Mac_Buffer[4];
		u32 Reg_Stat_Buffer[4];

		u32 ClipReg_Index;
		u64 ClipReg_BusyUntil_Cycle[4];
		u32 Reg_Clip_Buffer[4];
		...
		// adding storage for the current q-flag so stat flag only needs to be updated before reading/writing
		u32 CurQ_Flag;

		
		
		// need to make the current instruction more accesible, so it can be seen if M bit is set or not more easily
		Vu::Instruction::Format CurInstLO;
		Vu::Instruction::Format CurInstHI;
		
		Vu::Instruction::Format64 CurInstLOHI;
		...
		// set floating point destination register and each active component to be busy in vu pipeline
		inline void Set_FDstReg_Wait(u32 Instruction, u32 Register0)
		{
			//u32 xyzw32 = (Instruction >> 21) & 0xf;
			Vu::Instruction::Format xyzw32 = { Instruction };
			u64 ullBusyUntilCycle = CycleCount + 4;

			if (Register0)
			{
				// set cycle# register will be ready at (for vu0 macro mode and xyzw=0xf)
				FReg_BusyUntil[Register0] = ullBusyUntilCycle;

				// set cycle# each component will be ready at
				if (xyzw32.destx) FRegXYZW_BusyUntil[(Register0 << 2) + 0] = ullBusyUntilCycle;
				if (xyzw32.desty) FRegXYZW_BusyUntil[(Register0 << 2) + 1] = ullBusyUntilCycle;
				if (xyzw32.destz) FRegXYZW_BusyUntil[(Register0 << 2) + 2] = ullBusyUntilCycle;
				if (xyzw32.destw) FRegXYZW_BusyUntil[(Register0 << 2) + 3] = ullBusyUntilCycle;
			}
		}

		// set integer reg wait time - like for the integer destination in a load instruction
		inline void Set_IDstReg_Wait(u32 Register0)
		{
			if (Register0)
			{
				IReg_BusyUntil[Register0] = CycleCount + 4;
			}
		}


		inline void Wait_FSrcReg(u32 Instruction, u32 Register0)
		{
			//u32 xyzw32 = (Instruction >> 21) & 0xf;
			Vu::Instruction::Format xyzw32 = { Instruction };
			...
			if (Register0)
			{
				if (xyzw32.destx) { if (CycleCount < FRegXYZW_BusyUntil[(Register0 << 2) + 0]) CycleCount = FRegXYZW_BusyUntil[(Register0 << 2) + 0]; }
				if (xyzw32.desty) { if (CycleCount < FRegXYZW_BusyUntil[(Register0 << 2) + 1]) CycleCount = FRegXYZW_BusyUntil[(Register0 << 2) + 1]; }
				if (xyzw32.destz) { if (CycleCount < FRegXYZW_BusyUntil[(Register0 << 2) + 2]) CycleCount = FRegXYZW_BusyUntil[(Register0 << 2) + 2]; }
				if (xyzw32.destw) { if (CycleCount < FRegXYZW_BusyUntil[(Register0 << 2) + 3]) CycleCount = FRegXYZW_BusyUntil[(Register0 << 2) + 3]; }
			}
		}

		inline void Wait_FSrcRegBC(u32 Instruction, u32 RegisterBC)
		{
			//u32 xyzw32 = 0x8 >> (Instruction & 0x3);
			Vu::Instruction::Format xyzw32 = { (u32)(0x8 >> (Instruction & 0x3)) << 21 };

			if (RegisterBC)
			{
				if (xyzw32.destx) { if (CycleCount < FRegXYZW_BusyUntil[(RegisterBC << 2) + 0]) CycleCount = FRegXYZW_BusyUntil[(RegisterBC << 2) + 0]; }
				if (xyzw32.desty) { if (CycleCount < FRegXYZW_BusyUntil[(RegisterBC << 2) + 1]) CycleCount = FRegXYZW_BusyUntil[(RegisterBC << 2) + 1]; }
				if (xyzw32.destz) { if (CycleCount < FRegXYZW_BusyUntil[(RegisterBC << 2) + 2]) CycleCount = FRegXYZW_BusyUntil[(RegisterBC << 2) + 2]; }
				if (xyzw32.destw) { if (CycleCount < FRegXYZW_BusyUntil[(RegisterBC << 2) + 3]) CycleCount = FRegXYZW_BusyUntil[(RegisterBC << 2) + 3]; }
			}
		}

		inline void Wait_ISrcReg(u32 Register0)
		{
			if (Register0)
			{
				if (CycleCount < IReg_BusyUntil[Register0])
				{
					CycleCount = IReg_BusyUntil[Register0];
				}
			}
		}

		inline void Check_PReg()
		{
			//if (PBusyUntil_Cycle)
			{
				// check for the p-register to be ready
				if (CycleCount >= PBusyUntil_Cycle)
				{
					// set the p-register
					vi[REG_P].s = NextP.l;
		...
		inline void Check_QReg()
		{
			//if (QBusyUntil_Cycle)
			{
				// check for the q-register to be ready
				if (CycleCount >= QBusyUntil_Cycle)
				{
					// set the q-register
					vi[REG_Q].s = NextQ.l;

					// set the current q-flag
					...
					// set flags
					vi[REG_STATUSFLAG].uLo |= NextQ_Flag;
					//CurQ_Flag |= NextQ_Flag;

					// clear sticky NextQ_Flag so that it does not reset sticky flags
					NextQ_Flag &= ~0xc00;
		...
		inline void Wait_PReg()
		{
			// what if P-register gets set to a particular value? this way takes this into account
			// can also clear PBusyUntil_Cycle if it gets set to a value
			//if (PBusyUntil_Cycle)
			{
				// wait for the p-register to be ready
				if (CycleCount < PBusyUntil_Cycle)
				{
					CycleCount = PBusyUntil_Cycle;
				}
		...
		inline void Wait_QReg()
		{
			//if (QBusyUntil_Cycle)
			{
				// wait for the q-register to be ready
				if (CycleCount < QBusyUntil_Cycle)
				{
					CycleCount = QBusyUntil_Cycle;
				}
		...
		inline u32 Retrieve_ClipFlag()
		{
			u32 CurFlag = vi[REG_CLIPFLAG].u;
			u64 CurCycle = 0;
			u64 CompareCycle = CycleCount - 4;

			// the comparisons are presented in the order they would happen in x64 assembly
			if ((CompareCycle >= FlagSave[0].ullBusyUntil_Cycle) && (CurCycle < FlagSave[0].ullBusyUntil_Cycle)) { CurFlag = FlagSave[0].ClipFlag; CurCycle = FlagSave[0].ullBusyUntil_Cycle; }
			if ((CompareCycle >= FlagSave[1].ullBusyUntil_Cycle) && (CurCycle < FlagSave[1].ullBusyUntil_Cycle)) { CurFlag = FlagSave[1].ClipFlag; CurCycle = FlagSave[1].ullBusyUntil_Cycle; }
			if ((CompareCycle >= FlagSave[2].ullBusyUntil_Cycle) && (CurCycle < FlagSave[2].ullBusyUntil_Cycle)) { CurFlag = FlagSave[2].ClipFlag; CurCycle = FlagSave[2].ullBusyUntil_Cycle; }
			if ((CompareCycle >= FlagSave[3].ullBusyUntil_Cycle) && (CurCycle < FlagSave[3].ullBusyUntil_Cycle)) { CurFlag = FlagSave[3].ClipFlag; }

			return CurFlag;
		}

		inline u32 Retrieve_MacFlag()
		{
			u32 CurFlag = vi[REG_MACFLAG].u;
			u64 CurCycle = 0;
			u64 CompareCycle = CycleCount - 4;

			// the comparisons are presented in the order they would happen in x64 assembly
			if ((CompareCycle >= FlagSave[0].ullBusyUntil_Cycle) && (CurCycle < FlagSave[0].ullBusyUntil_Cycle)) { CurFlag = FlagSave[0].MACFlag; CurCycle = FlagSave[0].ullBusyUntil_Cycle; }
			if ((CompareCycle >= FlagSave[1].ullBusyUntil_Cycle) && (CurCycle < FlagSave[1].ullBusyUntil_Cycle)) { CurFlag = FlagSave[1].MACFlag; CurCycle = FlagSave[1].ullBusyUntil_Cycle; }
			if ((CompareCycle >= FlagSave[2].ullBusyUntil_Cycle) && (CurCycle < FlagSave[2].ullBusyUntil_Cycle)) { CurFlag = FlagSave[2].MACFlag; CurCycle = FlagSave[2].ullBusyUntil_Cycle; }
			if ((CompareCycle >= FlagSave[3].ullBusyUntil_Cycle) && (CurCycle < FlagSave[3].ullBusyUntil_Cycle)) { CurFlag = FlagSave[3].MACFlag; }

			return CurFlag;
		}

		inline u32 Retrieve_StatFlag()
		{
			u32 CurFlag = vi[REG_STATUSFLAG].u;
			u64 CurCycle = 0;
			u64 CompareCycle = CycleCount - 4;

			// the comparisons are presented in the order they would happen in x64 assembly
			if ((CompareCycle >= FlagSave[0].ullBusyUntil_Cycle) && (CurCycle < FlagSave[0].ullBusyUntil_Cycle)) { CurFlag = FlagSave[0].StatusFlag; CurCycle = FlagSave[0].ullBusyUntil_Cycle; }
			if ((CompareCycle >= FlagSave[1].ullBusyUntil_Cycle) && (CurCycle < FlagSave[1].ullBusyUntil_Cycle)) { CurFlag = FlagSave[1].StatusFlag; CurCycle = FlagSave[1].ullBusyUntil_Cycle; }
			if ((CompareCycle >= FlagSave[2].ullBusyUntil_Cycle) && (CurCycle < FlagSave[2].ullBusyUntil_Cycle)) { CurFlag = FlagSave[2].StatusFlag; CurCycle = FlagSave[2].ullBusyUntil_Cycle; }
			if ((CompareCycle >= FlagSave[3].ullBusyUntil_Cycle) && (CurCycle < FlagSave[3].ullBusyUntil_Cycle)) { CurFlag = FlagSave[3].StatusFlag; }
