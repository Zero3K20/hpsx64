...
#define ALLOW_VERBOSE_DEBUG_PS2_IPU_READ
#define ALLOW_VERBOSE_DEBUG_PS2_IPU_WRITE
...
bool IPU::bEnableVerboseDebugRead;
bool IPU::bEnableVerboseDebugWrite;
...
u64 IPU::Read ( u32 Address, u64 Mask )
{
...
#ifdef ALLOW_VERBOSE_DEBUG_PS2_IPU_READ
	if (bEnableVerboseDebugRead)
	{
		cout << "\nPS2 IPU READ ADDR:0x" << hex << Address;

		if ((Address & 0xffff) < 0x2040)
		{
			cout << " " << REG_NAMES[(Address >> 4) & 0x3];
		}
		else if ((Address & 0xffff) >= 0x7000 && (Address & 0xffff) < 0x8000)
		{
			cout << " " << REG_NAMES_FIFO[(Address >> 4) & 0x1];
		}
		else
		{
			cout << " ERROR";
...
#endif	// end #ifdef ALLOW_VERBOSE_DEBUG_PS2_IPU_READ


	if ((Address & 0xffff) < 0x3000)
	{
	...
	else if ((Address & 0xffff) < 0x8000 && ((Address >> 4) & 0xf) == 0)
	{
		if (!Mask)
		{
...
#ifdef ALLOW_VERBOSE_DEBUG_PS2_IPU_WRITE
	if (bEnableVerboseDebugWrite)
	{
		cout << "\nPS2 IPU WRITE ADDR:0x" << hex << Address;
...
#endif	// end #ifdef ALLOW_VERBOSE_DEBUG_PS2_IPU_WRITE
...
//#endif
	...
	else if ((Address & 0xffff) < 0x8000 && ((Address >> 4) & 0xf) == 1)
	{
#ifdef INLINE_DEBUG_WRITE
		debug << "; FIFOin";
#endif
...
void IPU::DebugWindow_Enable()
{
	...
	static constexpr const char* DebugWindow_Caption = "PS2 IPU Debug Window";
