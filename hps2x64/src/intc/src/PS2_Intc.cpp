...
#define ALLOW_VERBOSE_DEBUG_PS2_INTC_READ
#define ALLOW_VERBOSE_DEBUG_PS2_INTC_WRITE
...
u64 Intc::Read ( u32 Address, u64 Mask )
{
...
#ifdef ALLOW_VERBOSE_DEBUG_PS2_INTC_READ
	if (bEnableVerboseDebugRead)
	{
		cout << "\nPS2 INTC READ ADDR:0x" << hex << Address;

		if ((Address & 0xffff) < 0xf020)
		{
			cout << " " << REG_NAMES[(Address >> 4) & 0x1];
		}
		else
		{
			cout << " ERROR";
...
#endif	// end #ifdef ALLOW_VERBOSE_DEBUG_PS2_INTC_READ
	...
	if (bEnableVerboseDebugRead)
	{
		cout << " DATA:0x" << hex << ulOutput << " MASK:0x" << Mask << " PC:0x" << *_DebugPC << " CYCL#" << dec << *_DebugCycleCount;
	}
#endif
...
#ifdef ALLOW_VERBOSE_DEBUG_PS2_INTC_WRITE
	if (bEnableVerboseDebugWrite)
	{
		cout << "\nPS2 INTC WRITE ADDR:0x" << hex << Address;
...
#endif	// end #ifdef ALLOW_VERBOSE_DEBUG_PS2_INTC_WRITE
