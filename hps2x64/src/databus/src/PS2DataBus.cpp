...
#define ENABLE_PS2_VIRTUAL_MACHINE_MAINMEMORY
#define ENABLE_PS2_VIRTUAL_MACHINE_BIOS
#define ENABLE_PS2_VIRTUAL_MACHINE_SCRATCHPAD
...
DataBus::MemoryPtr_Format DataBus::MainMemory;
DataBus::MemoryPtr_Format DataBus::BIOS;
DataBus::MemoryPtr_Format DataBus::ScratchPad;

char* DataBus::pRamVirtual;
char* DataBus::pBiosVirtual;
char* DataBus::pScratchPadVirtual;
...
void DataBus::Start ()
{
...
#ifdef ENABLE_PS2_VIRTUAL_MACHINE_MAINMEMORY


	/*
	//SIZE_T viewSize = 4096; // 4 KB
	std::vector<void*> targetAddresses = {
		(LPVOID)(0ull | 0x200000000ull),
		//(void*)0x40000000ull,
		//(void*)0x60000000ull,
		//(void*)0x70000000ull,
		//(void*)0x30000000
	...
	*/
	...
	// Map PS2 address ranges for Main Ram
	pMainMemoryViews = MemoryManager::MapSharedMemoryToSpecifiedAddresses(countof(R5900_MAIN_MEMORY_SECTIONS), R5900_MAIN_MEMORY_SECTIONS, R5900_MAIN_MEMORY_SIZE_BYTES, R5900_MAIN_MEMORY_PERMISSIONS);
	

	/*
	if (!views.empty()) {
		strcpy(static_cast<char*>(views[0]), "Aliased memory!");

		std::cout << "View 2 reads: " << static_cast<char*>(views[1]) << "\n";
		std::cout << "View 3 reads: " << static_cast<char*>(views[2]) << "\n";
	...
	*/
	...
	// ps2 main ram is 32MB
	// ***TODO*** for now doing size+1 due to memory overruns, because sometimes dma transfers extend past edge of memory
	//pRamVirtual = (char*)VirtualAlloc((LPVOID)(0ull | 0x200000000ull), Playstation2::DataBus::MainMemory_Size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);

	pRamVirtual = (char*)(pMainMemoryViews[0]);
	if (pRamVirtual)
	{
		cout << "\nhpsx64: VIRTUAL MACHINE: PS2 MAIN RAM Allocated successfully.";
	}
#endif

#ifdef ENABLE_PS2_VIRTUAL_MACHINE_BIOS
	// ps2 bios is 4MB
	//pBiosVirtual = (char*)VirtualAlloc((LPVOID)(Playstation2::DataBus::BIOS_Start | 0x200000000ull), Playstation2::DataBus::BIOS_Size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);

	// Map PS2 address ranges for BIOS
	pBiosMemoryViews = MemoryManager::MapSharedMemoryToSpecifiedAddresses(countof(R5900_BIOS_MEMORY_SECTIONS), R5900_BIOS_MEMORY_SECTIONS, R5900_BIOS_MEMORY_SIZE_BYTES, R5900_BIOS_MEMORY_PERMISSIONS);

	pBiosVirtual = (char*)(pBiosMemoryViews[0]);
	if (pBiosVirtual)
	{
		cout << "\nhpsx64: VIRTUAL MACHINE: PS2 BIOS Allocated successfully.";
	}
#endif

#ifdef ENABLE_PS2_VIRTUAL_MACHINE_SCRATCHPAD
	// must mask addresses on ps2 with 0x5fffffff when using virtual machine instead of 0x1fffffff like on ps1
	//pScratchPadVirtual = (char*)VirtualAlloc((LPVOID)((Playstation2::DataBus::ScratchPad_Start & 0x5fffffff) | 0x200000000ull), Playstation2::DataBus::ScratchPad_Size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);

	pScratchPadMemoryViews = MemoryManager::MapSharedMemoryToSpecifiedAddresses(countof(R5900_SCRATCHPAD_MEMORY_SECTIONS), R5900_SCRATCHPAD_MEMORY_SECTIONS, R5900_SCRATCHPAD_MEMORY_SIZE_BYTES, R5900_SCRATCHPAD_MEMORY_PERMISSIONS);

	pScratchPadVirtual = (char*)(pScratchPadMemoryViews[0]);
	if (pScratchPadVirtual)
	{
		cout << "\nhpsx64: VIRTUAL MACHINE: PS2 SCRATCHPAD RAM Allocated successfully.";
	}
#endif
...
#ifdef ENABLE_PS2_VIRTUAL_MACHINE_MAINMEMORY
	MainMemory.b32 = (u32*)pRamVirtual;
#else
	MainMemory.b32 = PhyMainMemory.b32;
#endif

#ifdef ENABLE_PS2_VIRTUAL_MACHINE_BIOS
	BIOS.b32 = (u32*)pBiosVirtual;
#else
	// set the arrays to now be pointers
	BIOS.b32 = PhyBIOS.b32;
#endif

#ifdef ENABLE_PS2_VIRTUAL_MACHINE_SCRATCHPAD
	ScratchPad.b32 = (u32*)pScratchPadVirtual;
#else
	// set the arrays to now be pointers
	ScratchPad.b32 = PhyScratchPad.b32;
#endif
...
DataBus::~DataBus ()
{
	...
	// ***TODO*** for now doing size+1 due to memory overruns, because sometimes dma transfers extend past edge of memory
	VirtualUnlock((LPVOID)pRamVirtual, DataBus::MainMemory_Size);
	if (!VirtualFree((LPVOID)pRamVirtual, NULL, MEM_RELEASE))
	{
		cout << "\nhpsx64: VIRTUAL MACHINE: PROBLEM freeing Virtual RAM.";
	}
#endif

#ifdef ENABLE_PS2_VIRTUAL_MACHINE_BIOS
	VirtualUnlock((LPVOID)pBiosVirtual, DataBus::BIOS_Size);
	if (!VirtualFree((LPVOID)pBiosVirtual, NULL, MEM_RELEASE))
	{
		cout << "\nhpsx64: VIRTUAL MACHINE: PROBLEM freeing Virtual BIOS.";
	}
#endif

#ifdef ENABLE_PS2_VIRTUAL_MACHINE_SCRATCHPAD

	VirtualUnlock((LPVOID)pScratchPadVirtual, DataBus::ScratchPad_Size);
	if (!VirtualFree((LPVOID)pScratchPadVirtual, NULL, MEM_RELEASE))
	{
		cout << "\nhpsx64: VIRTUAL MACHINE: PROBLEM freeing Virtual ScratchPad.";
	}
#endif
...
void* DataBus::GetPointer ( u32 Address )
{
	...
	{
		return (void*) &(BIOS.b8 [ Address & BIOS_Mask]);
	}
	else
	{
		return (void*) &(MainMemory.b8 [ Address & MainMemory_Mask]);
	}
}
...
void DataBus::SaveVirtualState()
{
#ifdef ENABLE_PS2_VIRTUAL_MACHINE_MAINMEMORY

	memcpy((void*)PhyMainMemory.b8, (const void*)MainMemory.b8, MainMemory_Size);

#endif

#ifdef ENABLE_PS2_VIRTUAL_MACHINE_BIOS

	memcpy((void*)PhyBIOS.b8, (const void*)BIOS.b8, BIOS_Size);

#endif

#ifdef ENABLE_PS2_VIRTUAL_MACHINE_SCRATCHPAD

	memcpy((void*)PhyScratchPad.b8, (const void*)ScratchPad.b8, ScratchPad_Size);
...
void DataBus::RestoreVirtualState()
{
#ifdef ENABLE_PS2_VIRTUAL_MACHINE_MAINMEMORY

	memcpy((void*)MainMemory.b8, (const void*)PhyMainMemory.b8,  MainMemory_Size);

#endif

#ifdef ENABLE_PS2_VIRTUAL_MACHINE_BIOS

	memcpy((void*)BIOS.b8, (const void*)PhyBIOS.b8,  BIOS_Size);

#endif

#ifdef ENABLE_PS2_VIRTUAL_MACHINE_SCRATCHPAD

	memcpy((void*)ScratchPad.b8, (const void*)PhyScratchPad.b8,  ScratchPad_Size);
...
void DataBus::DebugWindow_Enable ()
{
	...
	static constexpr const char* DebugWindow_Caption = "PS2 Memory Bus Debug Window";
