...
std::map<std::string, WindowClass::MenuBar::Function> mapped_functions;

void WindowClass::MenuBar::add_map_function(const char* sName, WindowClass::MenuBar::Function pfunc_ptr)
{
	mapped_functions[sName] = pfunc_ptr;
}
WindowClass::MenuBar::Function WindowClass::MenuBar::get_map_function(const char* sName)
{
	return mapped_functions[sName];
}
...
#ifdef VERBOSE_CREATE_MENU_FROM_JSON
...
string WindowClass::Window::ShowFileOpenDialog (const char* sFileFilter)
{
	...
	ofn.lpstrFilter = sFileFilter;	// "All\0*.*\0Text\0*.TXT\0";
	ofn.nFilterIndex = 1;
...
BOOL WindowClass::Window::CreateWindowWithMenu(const char* title, int width, int height, bool has_menu, bool fullscreenflag)
{
...
bool WindowClass::Window::CreateBaseWindow(const char* title, int width, int height, bool has_menu)
{
...
bool WindowClass::Window::KillWindow()
{
	if (fullscreen)										// Are We In Fullscreen Mode?
	{
		ChangeDisplaySettings(NULL, 0);					// If So Switch Back To The Desktop
	...
	// destroy the menu
	Menus->ListOfMenuBarItems.clear();
	Menus->ListOfMenuBars.clear();
	if (!DestroyMenu(Menus->MainMenu))
	{
		MessageBox(NULL, "Destroy Menu Failed.", "SHUTDOWN ERROR", MB_OK | MB_ICONINFORMATION);
	}
	delete Menus;
	Menus = nullptr;
...
BOOL WindowClass::Window::CreateOpenGLContext()
{
...
BOOL WindowClass::Window::DestroyOpenGLContext()
{
	HGLRC    hglrc;
	HDC      hdc;
	...
	if (hglrc = wglGetCurrentContext())
	{
		...
		if (!wglDeleteContext(hRC))				// Are We Able To Get A Rendering Context?
		{
			KillGLWindow();								// Reset The Display
			MessageBox(NULL, "Can't Delete A GL Rendering Context.", "ERROR", MB_OK | MB_ICONEXCLAMATION);
			return FALSE;								// Return FALSE
		}
		...
		if (!DeleteDC(hDC))
		{
			cout << "\nWinApiHandler: ERROR: Problem deleting OpenGL device context.";
		}
