...
// allow verbose debugging to the console
#define ALLOW_VERBOSE_DEBUG_PS1_DMA_READ
#define ALLOW_VERBOSE_DEBUG_PS1_DMA_WRITE
...
void Dma::Reset ()
{
	...
	// init static vars
	bEnableVerboseDebugRead = false;
	bEnableVerboseDebugWrite = false;
...
u32 Dma::Priority ( int iChannel )
{
...
#ifdef ENABLE_START_CYCLE
	// check if we have reached start cycle# for channel
	if (DmaCh[iChannel].ullStartCycle > *_DebugCycleCount)
	{
#ifdef INLINE_DEBUG_PRIORITY
		debug << "(!START)";
#endif
	...
	*/
	...
	return (((iChannel & 0xff) << 8) | ((~ulPriority & 0xff) << 16) | ulPrReady);
#endif
}
...
//#endif
...
#ifdef ENABLE_START_CYCLE
		// set start cycle since the channel will not be ready again until that cycle#
		DmaCh[iChannel].ullStartCycle = ullReady;
#endif
	...
	*/
	...
	debug << " WC=" << dec << WordCount;
				...
				if ( WordCount > 32 )
				{
					cout << "\nhps1x64: GPU: ALERT: WC>32 for linked-list transfer to GPU. WC=" << dec << WordCount;
					cout << " BufferMode=" << hex << _GPU->BufferMode;
				}
				...
				//if ( TransferAmt )
				{
					...
					// note: don't want the release time for transferring to be the same for transferring commands
...
#ifdef INLINE_DEBUG_TRANSFER_FROMMEM_LOOKAHEAD
				debug << " LOOKAHEAD=" << hex << " NextAddress=" << hex << (NextAddress & DataBus::MainMemory_Mask);
				for (int i = 0; i < 16; i++)
				{
					debug << hex << " " << pMemoryPtr[((NextAddress + (i << 2)) & DataBus::MainMemory_Mask) >> 2];
				}
#endif
...
#ifdef ENABLE_START_CYCLE
				// set next start cycle#
				DmaCh[iChannel].ullStartCycle = ullReturnCycle;
#endif
			}
			else
			{
	...
	else
	{
		...
		// check if device is busier longer or if release time is longer
		ullReturnCycle = ullReleaseTime + R3000A::Cpu::_CPU->CycleCount;

		//if (ullReturnCycle > ullReady)
		if (ullReady > ullReturnCycle)
		{
			ullReturnCycle = ullReady;
		}
		
#ifdef INLINE_DEBUG_TRANSFER_BOTTOM
	debug << " DEVICE-WILL-BE-READY-AT-CYCLE#" << dec << ullReturnCycle;
#endif
...
u32 Dma::Read(u32 Address)
{
...
#ifdef ALLOW_VERBOSE_DEBUG_PS1_DMA_READ
	if (bEnableVerboseDebugRead)
	{
		cout << std::endl << "PS1 DMA READ ADDR:0x" << hex << Address;

		if ((Address & 0xffff) < 0x1500)
		{
			cout << " " << REG_NAMES[(Address >> 2) & 0x1f];
		}
#ifdef PS2_COMPILE
		else if ((Address & 0xffff) < 0x1600)
		{
			cout << " " << REG_NAMES2[(Address >> 2) & 0x1f];
		}
#endif
		else
		{
...
#endif	// end #ifdef ALLOW_VERBOSE_DEBUG_PS1_DMA_READ
	...
	{
#if defined INLINE_DEBUG_READ
		debug << " " << REG_NAMES[(Address - 0x1080) >> 2];
#endif
	...
	{
#if defined INLINE_DEBUG_READ
		debug << " " << REG_NAMES2[(Address - 0x1500) >> 2];
#endif
	...
	if (bEnableVerboseDebugRead)
	{
		cout << " DATA:0x" << hex << ulOutput << " PC:0x" << *_DebugPC << " CYCL#" << dec << *_DebugCycleCount;
	}
#endif
...
#ifdef ALLOW_VERBOSE_DEBUG_PS1_DMA_WRITE
	if (bEnableVerboseDebugWrite)
	{
		cout << std::endl << "PS1 DMA WRITE ADDR:0x" << hex << Address;
...
#endif	// end #ifdef ALLOW_VERBOSE_DEBUG_PS1_DMA_WRITE
		...
		if ( Address & 2 )
		{
			if ( Address < ( ( ( PCR & 0xffff ) - 0x1080 ) >> 2 ) )
			{
				...
				if ( ( _DMA->DMARegs0.Regs [ Address ] & 0x1000000 ) && !( PreviousValue & 0x1000000 ) )
				{
					...
					_DMA->DmaCh[iChannel].ullStartCycle = 0;
...
void Dma::DebugWindow_Enable ()
{
	...
	static constexpr const char* DebugWindow_Caption = "PS1 DMA Debug Window";
