...
// no need to invalidate memory on ps1 anymore since it checks for modified code in uncached areas
//#define ENABLE_MEMORY_INVALIDATE

#define ENABLE_PS1_VIRTUAL_MACHINE_MAINMEMORY
#define ENABLE_PS1_VIRTUAL_MACHINE_BIOS
...
#define INLINE_DEBUG_READ_CACHE_CONTROL
#define INLINE_DEBUG_WRITE_CACHE_CONTROL
...
#ifdef ENABLE_PS1_VIRTUAL_MACHINE_MAINMEMORY
	// ram is 2MB
	//pRamVirtual = (char*)VirtualAlloc((LPVOID)(0ull | 0x100000000ull), DataBus::MainMemory_Size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);

	// Map PS1 address ranges for Main Ram
	pMainMemoryViews = MemoryManager::MapSharedMemoryToSpecifiedAddresses(countof(R3000A_MAIN_MEMORY_SECTIONS), R3000A_MAIN_MEMORY_SECTIONS, R3000A_MAIN_MEMORY_SIZE_BYTES, R3000A_MAIN_MEMORY_PERMISSIONS);

	pRamVirtual = (char*)pMainMemoryViews[0];
	if (pRamVirtual)
	{
		cout << "\nhpsx64: VIRTUAL MACHINE: RAM Allocated successfully.";
	}
#endif

#ifdef ENABLE_PS1_VIRTUAL_MACHINE_BIOS
	// bios is 512KB
	//pBiosVirtual = (char*)VirtualAlloc((LPVOID)(DataBus::BIOS_Start | 0x100000000ull), DataBus::BIOS_Size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);

	pBiosMemoryViews = MemoryManager::MapSharedMemoryToSpecifiedAddresses(countof(R3000A_BIOS_MEMORY_SECTIONS), R3000A_BIOS_MEMORY_SECTIONS, R3000A_BIOS_MEMORY_SIZE_BYTES, R3000A_BIOS_MEMORY_PERMISSIONS);

	pBiosVirtual = (char*)pBiosMemoryViews[0];
	if (pBiosVirtual)
	{
		cout << "\nhpsx64: VIRTUAL MACHINE: BIOS Allocated successfully.";
	}
#endif
...
void* DataBus::GetIMemPtr(u32 Address)
{
	if (((Address >> 20) & 0x1fc) == 0x1fc)
	{
		Latency = c_iBIOS_Read_Latency;
...
u32 DataBus::InvalidAddress_Read ( u32 Address )
{
	...
	{
#ifdef INLINE_DEBUG_READ_CACHE_CONTROL
		debug << "\r\nBus::Read; PC=" << hex << setw(8) << *_DebugPC << " " << dec << *_DebugCycleCount << "; Address=" << hex << Address;
		debug << ";CACHE-CONTROL; READ; ADDRESS = " << hex << setw(8) << Address;
#endif
...
void DataBus::InvalidAddress_Write ( u32 Address, u32 Data, u32 Mask )
{
	if ((Address >> 16) == 0xfffe)
	{
#ifdef INLINE_DEBUG_WRITE_CACHE_CONTROL
		debug << "\r\nBus::Write; PC=" << hex << setw(8) << *_DebugPC << " " << dec << *_DebugCycleCount << "; Address=" << hex << Address << "; Data=" << Data;
		debug << ";CACHE-CONTROL; WRITE; ADDRESS = " << hex << setw(8) << Address;
#endif
...
void DataBus::DebugWindow_Enable ()
{
	...
	static constexpr const char* DebugWindow_Caption = "PS1 Memory Bus Debug Window";
