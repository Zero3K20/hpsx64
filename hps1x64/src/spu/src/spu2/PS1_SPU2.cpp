...
#define ALLOW_VERBOSE_DEBUG_PS1_SPU2_READ
#define ALLOW_VERBOSE_DEBUG_PS1_SPU2_WRITE
...
bool SPU2::bEnableVerboseDebugRead;
bool SPU2::bEnableVerboseDebugWrite;
...
WaveOutDriver* SPU2::waveout_driver;
...
void SPUCore::Reset ()
{
	...
	*/
	...
	SPU2::bEnableVerboseDebugRead = false;
	SPU2::bEnableVerboseDebugWrite = false;
...
u32 SPUCore::Read ( u32 Offset, u32 Mask )
{
	...
	if ( ROffset < 0x346 )
	{
		debug << " " << REG_NAMES0[ ROffset >> 1 ];
	}
	else if ( ROffset >= 0x760 && ROffset < 0x788 )
	{
		debug << " " << REG_NAMES1[ ( ROffset - 0x760 ) >> 1 ];
	}
#endif


#ifdef ALLOW_VERBOSE_DEBUG_PS1_SPU2_READ
	if (SPU2::bEnableVerboseDebugRead)
	{
		//cout << "\nPS1 SPU2 CORE#" << CoreNumber << " READ ADDR:0x" << hex << Address;
		cout << " CORE#" << CoreNumber;

		if (ROffset < 0x346)
		{
			cout << " " << REG_NAMES0[ROffset >> 1];
		}
		else if (ROffset >= 0x760 && ROffset < 0x788)
		{
			cout << " " << REG_NAMES1[(ROffset - 0x760) >> 1];
		}
		else
		{
			cout << " ERROR";
...
#endif	// end #ifdef ALLOW_VERBOSE_DEBUG_PS1_SPU2_READ
...
void SPUCore::Write ( u32 Offset, u32 Data, u32 Mask )
{
...
#ifdef ALLOW_VERBOSE_DEBUG_PS1_SPU2_WRITE
	if (SPU2::bEnableVerboseDebugWrite)
	{
		//cout << "\nPS1 TIMER WRITE ADDR:0x" << hex << Address;
		cout << " CORE#" << CoreNumber;
...
#endif	// end #ifdef ALLOW_VERBOSE_DEBUG_PS1_SPU2_WRITE
	...
	switch ( ROffset )
	{
		...
		case SBA_1:
			...
			//if ((pCoreRegs0->ADMAS & (1 << CoreNumber)))
			//{
			//	cout << "\n********************************";
			//	cout << "\nhps2x64: SPU: Core#" << CoreNumber << " SBA=" << NextSoundBufferAddress;
			//}
	...
	debug << "; MANUAL WRITE";
		...
		case ADMAS:
			...
			*/
			...
			//cout << "\n********************************";
			//cout << "\nhps2x64: SPU: Core#" << CoreNumber << " ADMAS=" << Data;
...
//#endif
	...
	{
//#define USE_BUFOFFSET_FOR_ADMA
#ifdef USE_BUFOFFSET_FOR_ADMA
		ulADMA_PlayOffset = DecodeBufferOffset;
#endif

		//if (((s64)ulADMA_PlayOffset) > ((s64)NextSoundBufferAddress))
		//{
		//	cout << "\nhps2x64: SPU: playoffset > nextsoundbufferaddress.\n";
		//}
		...
		//if (((s64)(NextSoundBufferAddress - ulADMA_PlayOffset)) > 0)
		{
			...
			//if (!(ulADMA_PlayOffset & 0x7f))
			{
				// clear the opposite buffer-half for a transfer since it is done playing
				//bBufferFull [ ( ulADMA_PlayOffset >> 9 ) ^ 1 ] = 0;

				// if going to transfer to the other buffer, then transfer
				//if ((ulADMA_PlayOffset ^ (NextSoundBufferAddress << 1)) & 0x200)
				//if (((s64)(NextSoundBufferAddress - ulADMA_PlayOffset)) <= 128)
				if (((s64)(NextSoundBufferAddress - ulADMA_PlayOffset)) <= 512)
				{
...
#endif	// end #ifdef ENABLE_AVOL_CALC
...
//debug << "\r\nREVERB-OUT-RIGHT-CH#" << dec << Channel << "-CORE#" << dec << CoreNumber << "-SAMPLE:" << hex << ReverbR_Output;
	...
	// 256 16-bit samples in each half of buffer, 512 16-bit samples total
...
bool SPUCore::DMA_ReadyForWrite ( void )
{
	...
	if ( isADMATransferMode () )
	{
		...
		//if ( ((s64) (NextSoundBufferAddress - ulADMA_PlayOffset)) <= 512 )
		if (((u64)(NextSoundBufferAddress - ulADMA_PlayOffset)) <= 512)
		{
#ifdef INLINE_DEBUG_DMA_WRITE
...
u32 SPU2::Read ( u32 Address )
{
	...
	{
		cout << "\nPS1 SPU2 READ ADDR:0x" << hex << Address;
	}
#endif	// end #ifdef ALLOW_VERBOSE_DEBUG_PS1_SPU2_READ
...
#ifdef INLINE_DEBUG_READ
			SPUCore::debug << "\r\nSPU::Read; " << hex << setw(8) << *_DebugPC << " " << dec << *_DebugCycleCount << hex << " Mask=" << Mask << " Offset= " << Offset;
			SPUCore::debug << "; " << SPUCore::REG_NAMES2[ ( Offset - 0x7c0 ) >> 1 ];
#endif

#ifdef VERBOSE_READ_TEST
			cout << "\nSPU::Read; " << hex << setw(8) << *_DebugPC << " " << dec << *_DebugCycleCount << hex << " Mask=" << Mask << " Offset= " << Offset;
			cout << "; " << SPUCore::REG_NAMES2[ ( Offset - 0x7c0 ) >> 1 ];
#endif
		}
	...
	if (bEnableVerboseDebugRead)
	{
		cout << " DATA:0x" << hex << Output << " PC:0x" << *_DebugPC << " CYCL#" << dec << *_DebugCycleCount;
	}
#endif
...
void SPU2::Write ( u32 Address, u32 Data, u32 Mask )
{
	...
	{
		cout << "\nPS1 SPU2 WRITE ADDR:0x" << hex << Address;
	}
#endif	// end #ifdef ALLOW_VERBOSE_DEBUG_PS1_SPU2_WRITE
...
void SPUCore::DebugWindow_Enable ( int Number )
{
	...
	static constexpr const char* DebugWindow_Caption = "SPU1 Debug Window";
