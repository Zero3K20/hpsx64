...
signed short *adpcm_decoder::decode_packet(adpcm_packet *ap, signed short *dp)
{
	...
	for (int i=0; i<14; i++)
	{
		...
		//short bl=(b&0xf)<<12,
					//bh=(b>>4)<<12;
		int32_t bl = sign_extend4 ( b & 0xf ) << shift,	//(b&0xf)<<12,
...
int32_t *adpcm_decoder::decode_packet32(adpcm_packet *ap, int32_t *dp)
{
	int32_t shift=ap->info&0xf,
			filter=ap->info>>4;
	int32_t f0=filter_coef[filter][0],
			f1=filter_coef[filter][1];
...
int32_t *adpcm_decoder::decode_packet_xa32 (unsigned char info, adpcm_packet_xa *ap, int32_t *dp)
{
	int32_t shift= info & 0xf,
			filter= ( info>>4 ) & 0xf;
	int32_t f0=filter_coef[filter][0],
			f1=filter_coef[filter][1];
