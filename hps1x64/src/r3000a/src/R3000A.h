...
namespace R3000A
{
	...
	public:
		...
		static constexpr const int R3000A_INSTRUCTIONS_PER_CACHE_ENTRY_SHIFT = 2;
		static constexpr const int R3000A_CODE_BLOCK_SIZE_SHIFT = 10;
		static constexpr const int R3000A_CODE_BLOCK_COUNT_SHIFT = 16;	// 9;
...
#ifdef PS2_COMPILE
		static const int32_t ClockSpeed = 36864000;
		static const int32_t ClockSpeed2 = 37375000;
#else
		...
		static constexpr u64 c_ullLatency_PipelineRefill = 2;
		...
		// re-arrange so that dcache is a pointer and can be placed in virtual memory
		static DCache_Ptr DCache;
		DCache_Format PhyDCache;

		static char* pDCacheVirtual;
		...
		// save/restore virtual memory data after/before saving/loading state
		...
		// for profiling
		void ExecuteRecompiledCode();
		void ExecuteSomethingBusy();
		void ExecuteNotCached();
		void ExecuteCacheMiss();
		void ExecuteExceptionBlock();
...
//		int32_t CycleTarget;	// this is the target number of cycles for the processor to execute
		...
		bool bDelaySlot0_Recompiler;
		u32 uDelaySlot0_TargetAddr;
...
static void ProcessSynchronousInterrupt_t ()
{
	...
	if ( _CPU->DelaySlots [ _CPU->NextDelaySlotIndex ].Value
		&&
		( _CPU->DelaySlots [ _CPU->NextDelaySlotIndex ].Instruction.Value < 0x40000000 )
		&&
		(_CPU->Status.DelaySlot_Valid & 1)
		)
	{
