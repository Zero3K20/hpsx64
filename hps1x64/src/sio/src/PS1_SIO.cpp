...
static constexpr const char* c_sMagic = "Sony PS2 Memory Card Format ";
static constexpr const char* c_sVersion = "1.2.0.0";
...
void SIO::Command_0x42 ( bool ForceAnalogOutput )
{
	...
	if(PortMap >= gamepad.GetDeviceCount())
	{
		PortMap = -1;
	}
	...
	switch ( PortMap )
	{
		...
		case 1:
			...
			temp_joy_state = SIO::gamepad.GetRawDeviceState(PortMap);

			if (temp_joy_state.rgdwPOV[0] > 0 && temp_joy_state.rgdwPOV[0] < 18000)
			{
				// right arrow is down on joystick
				Output_Buf[3] ^= 0x20;
			}
			if (temp_joy_state.rgdwPOV[0] > 9000 && temp_joy_state.rgdwPOV[0] < 27000)
			{
				// down arrow is down on joystick
				Output_Buf[3] ^= 0x40;
			}

			if (temp_joy_state.rgdwPOV[0] > 18000 && temp_joy_state.rgdwPOV[0] <= 36000)
			{
				// left arrow is down on joystick
				Output_Buf[3] ^= 0x80;
			}

			if ((temp_joy_state.rgdwPOV[0] < 9000 || temp_joy_state.rgdwPOV[0] > 27000) && temp_joy_state.rgdwPOV[0] <= 36000 && temp_joy_state.rgdwPOV[0] >= 0)
			{
			...
			if (temp_joy_state.rgbButtons[Key_Start[PadNum]])
			{
				// start key //
				Output_Buf[3] ^= 0x08;
			}

			if (temp_joy_state.rgbButtons[Key_Select[PadNum]])
			{
				// select key //
				Output_Buf[3] ^= 0x01;
			}

			if (temp_joy_state.rgbButtons[Key_X[PadNum]])
			{
				// x key //
				Output_Buf[4] ^= 0x40;
			}

			if (temp_joy_state.rgbButtons[Key_O[PadNum]])
			{
				// o key //
				Output_Buf[4] ^= 0x20;
			}

			if (temp_joy_state.rgbButtons[Key_Square[PadNum]])
			{
				// square key //
				Output_Buf[4] ^= 0x80;
			}

			if (temp_joy_state.rgbButtons[Key_Triangle[PadNum]])
			{
				// triangle key //
				Output_Buf[4] ^= 0x10;
			}

			if (temp_joy_state.rgbButtons[Key_L1[PadNum]])
			{
				// L1 key //
				Output_Buf[4] ^= 0x4;
			}

			if (temp_joy_state.rgbButtons[Key_L2[PadNum]])
			{
				// L2 key //
				Output_Buf[4] ^= 0x1;
			}

			if (temp_joy_state.rgbButtons[Key_R1[PadNum]])
			{
				// R1 key //
				Output_Buf[4] ^= 0x8;
			}

			if (temp_joy_state.rgbButtons[Key_R2[PadNum]])
			{
				// R2 key //
				Output_Buf[4] ^= 0x2;
			}

			if (temp_joy_state.rgbButtons[Key_L3[PadNum]])
			{
				// L3 key //
				Output_Buf[3] ^= 0x2;
			}

			if (temp_joy_state.rgbButtons[Key_R3[PadNum]])
			{
				// R3 key //
				Output_Buf[3] ^= 0x4;
			}

			//long* pData = (long*)&SIO::gamepad.GetRawDeviceState(PortMap);
			long* pData = (long*)&temp_joy_state;
