...
// does hblank signal occur during vblank??
//#define DISABLE_HBLANK_WITHIN_VBLANK
...
// mask final timer values
//#define ENABLE_MASK_FINAL_TIMER_VALUES


#define ALLOW_VERBOSE_DEBUG_PS1_TIMER_READ
#define ALLOW_VERBOSE_DEBUG_PS1_TIMER_WRITE
...
bool Timers::bEnableVerboseDebugRead;
bool Timers::bEnableVerboseDebugWrite;
...
#ifdef ALLOW_VERBOSE_DEBUG_PS1_TIMER_READ
	if (bEnableVerboseDebugRead)
	{
		cout << "\nPS1 TIMER READ ADDR:0x" << hex << Address;

		if ((Address & 0xffff) < 0x1480)
		{
			cout << " " << REG_NAMES[(Address >> 2) & 0xf];
		}
#ifdef PS2_COMPILE
		else if ((Address & 0xffff) < 0x1500)
		{
			cout << " " << REG_NAMES2[(Address >> 2) & 0xf];
		}
#endif
		else
		{
...
#endif	// end #ifdef ALLOW_VERBOSE_DEBUG_PS1_TIMER_READ
...
#endif
	{
		switch ( Address & 0xf )
		{
...
#ifdef ENABLE_MASK_FINAL_TIMER_VALUES
				ulOutput = _TIMERS->TheTimers[TimerNumber].COUNT.Value & ((TimerNumber < 3) ? 0xffffULL : 0xffffffffULL);
#else
				ulOutput = _TIMERS->TheTimers [ TimerNumber ].COUNT.Value;
#endif
				break;
			...
			case 4:	// READ: MODE
				...
				ulOutput = _TIMERS->TheTimers [ TimerNumber ].MODE.Value;
	...
	if ( TimerNumber == 1 )
	{
		...
		Output &= ~1;
			...
			case 8:	// READ: COMP
				...
				ulOutput = _TIMERS->TheTimers [ TimerNumber ].COMP.Value;
				break;
...
#ifdef ALLOW_VERBOSE_DEBUG_PS1_TIMER_WRITE
...
#endif	// end #ifdef ALLOW_VERBOSE_DEBUG_PS1_TIMER_WRITE
...
void Timer::SetMode( u32 Data )
{
	...
	// from Nocash PSX Specifications
	// writing to mode register sets bit 10 (InterruptRequest=Yes=1)
...
u64 Timer::CountTicks_Scanline ( u64 lBeginCycle, u64 lEndCycle )
{
	...
	Timers::debug << " T0-SYSTEM-CLOCK";
				...
				case 3:
					...
					Timers::debug << " PPC=" << g->dPixelsPerCycle;
					Timers::debug << " CPP=" << g->dCyclesPerPixel;
					Timers::debug << " Cycles=" << dec << (lEndCycle - lBeginCycle);
					Timers::debug << " Pixels=" << dec << ((u64)(((double)lEndCycle) * g->dPixelsPerCycle)) - ((u64)(((double)lBeginCycle) * g->dPixelsPerCycle));
#endif

					// dot clock //
					return ((u64)(((double)lEndCycle) * g->dPixelsPerCycle)) - ((u64)(((double)lBeginCycle) * g->dPixelsPerCycle));
					break;
			}
...
#ifdef DISABLE_HBLANK_WITHIN_VBLANK
					// no hblank if in vblank area ??
					if (g->lScanline < g->VBlank_Y)
#endif
					{
						// hblank clock //
...
#ifdef DISABLE_HBLANK_WITHIN_VBLANK
					// no interrupt from hblank clock if in vblank ?
					if (g->lScanline >= g->VBlank_Y)
					{
						return -1LL;
					}
...
void Timers::DebugWindow_Enable ()
{
	...
	static constexpr const char* DebugWindow_Caption = "PS1 Timer Debug Window";
