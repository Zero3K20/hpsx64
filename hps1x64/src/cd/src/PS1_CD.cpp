...
//#endif
	...
	debug << "; AUTOPAUSE_CDDA";
				...
				//{
					...
					if ( !isDataSector ( pSectorDataBuffer ) )
					{
						...
						if ( !MODE.Speed )
						{
							// if this is a chd, then it swaps the bytes on audio tracks
							if (cd_image.bIsChdFile)
							{
								// disk image is a chd //

								u32 uTemp32;
								for (int i = 0; i < (c_SectorSize / 4); i++)
								{
									uTemp32 = ((u32*)pSectorDataBuffer)[i];
									uTemp32 = ((uTemp32 >> 8) & 0x00ff00ff) | ((uTemp32 << 8) & 0xff00ff00);
									SpuBuffer[(SpuBuffer_WriteIndex + i) & c_iSpuBuffer_Mask] = Apply_CDDA_Volume(uTemp32);
								}
							}
							else
							{
						...
						else
						{
							...
							if (cd_image.bIsChdFile)
							{
								...
								u32 uTemp32;
								for (int i = 0; i < (c_SectorSize / 8); i++)
								{
									uTemp32 = ((u32*)pSectorDataBuffer)[i << 1];
									uTemp32 = ((uTemp32 >> 8) & 0x00ff00ff) | ((uTemp32 << 8) & 0xff00ff00);
									SpuBuffer[(SpuBuffer_WriteIndex + i) & c_iSpuBuffer_Mask] = Apply_CDDA_Volume(uTemp32);
								}
							}
...
#endif
	//cout << "\n(Local) AMin=" << dec << (u32)cd_image.Current_AMin << " ASec=" << (u32)cd_image.Current_ASec << " AFrac=" << (u32)cd_image.Current_AFrac << " Min=" << (u32)cd_image.SubQ_Min << " Sec=" << (u32)cd_image.SubQ_Sec << " Frac=" << (u32)cd_image.SubQ_Frac;
...
#endif
	//cout << "\nAMin=" << dec << (u32)cd_image.Current_AMin << " ASec=" << (u32)cd_image.Current_ASec << " AFrac=" << (u32)cd_image.Current_AFrac;
	//cout << "\nisChd=" << dec << cd_image.bIsChdFile << " isCD=" << cd_image.bIsDiskCD << " hunknum=" << cd_image.chd_cur_hunknum;
	//cout << "\nData=" << hex << ((u32*)pSectorDataBuffer)[0] << " " << ((u32*)pSectorDataBuffer)[1] << " " << ((u32*)pSectorDataBuffer)[2] << " " << ((u32*)pSectorDataBuffer)[3];
	...
	debug << "; AMin=" << dec << (u32)AMin << " ASec=" << (u32)ASec << " AFrac=" << (u32)AFrac;
#endif
	//cout << "\nAMin=" << dec << (u32)AMin << " ASec=" << (u32)ASec << " AFrac=" << (u32)AFrac;
...
#endif
	//cout << "\nMin=" << dec << (u32)AMin << " Sec=" << (u32)ASec << " Frac=" << (u32)Frac;
...
u32 CD::DecodeXA_Sector ( int32_t* Output, char* FullSectorData )
{
...
void CD::DebugWindow_Enable ()
{
	...
	static constexpr const char* DebugWindow_Caption = "PS1 CD Debug Window";
